...
远端 V7 发布后，仍保存在浏览器本地的进行中 V6 审阅运行曾被错误地按当前 V7 重新计算其内容
绑定，页面因而显示“冻结内容快照校验失败”。修复提交
`df5487d` 改为按运行指针原有的 V6 快照和 SHA 校验其已发布的追加链，并继续逐项核对运行 ID
和五份正式来源身份；它不清除浏览器数据，也不允许篡改或跨运行复用绑定。

该提交在隔离 worktree 中完成锁定依赖安装和生产构建后，于 2026-08-29 原子部署到：

```text
/opt/linguan-prototype-v2/releases/20260829T092527Z-df5487d
```

部署前 `current` 是 `20260829T081950Z-c9ded57`，用于失败回滚。远端逐项核对了入口 HTML、哈希
CSS 与哈希 JS；reload 后服务保持 `active`，服务器本机首页和深层路由均成功，外部
`http://172.21.1.150:4173/` 返回 200。定向验证包括 V7→V6→V5 发布链 15/15、工作台运行时
55/55（含浏览器保存 V6 运行的回归场景）及生产构建。没有修改 V6、V7 内容或远端源码。
