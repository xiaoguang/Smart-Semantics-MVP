...
前端消费冻结 Bundle 与当前审阅 revision。默认主区是“审阅清单”：它以 Claim 为唯一主数据，只展示待核对任务、
关键业务结论、资料缺口和紧凑对象明细；“Markdown 文档”是当前 revision 的只读生成投影。每个结论内可展开
...
每个来源使用同一信息架构：`审阅清单`、`Markdown 文档`。前者回答“本次要判断什么”，后者回答“当前审阅结果如何写成完整文档”。审阅清单过滤 Markdown 标题、内部对象名、只有位置的记录和重复元数据，只保留待核对任务、关键业务结论、资料缺口和紧凑对象明细。每条结论原位承担单源链路“来源材料 → 审阅结论 → Markdown 段落”；跨来源的一致、互补、冲突和不同版本记录不一致只在独立的“本次读取发现”区域显示。

冻结来源永远不可编辑；当前 Demo 仅允许两条有精确 `sourceId + snapshotId + Claim + Block + Evidence` 映射的剧本化文字修订。用户只能选择保留当前结论，或采用推荐修改后微调该剧本显式允许的业务名称／业务说明；编辑器只呈现这些可改字段和确认后会变化的 Markdown 段落。首版可以使用更短的业务展示名，但用户确认或微调后的名称必须在清单、Markdown 和结论内依据中逐字保留，不能被展示层再次裁剪。SQL、DDL和来源位置保持冻结，完整性约束仅留在内部审计。保留决定写入审计但不创建 revision，采用推荐修改先产生纯预览，确认后通过既有 `SourceDocumentRuntime.revise → REVISE_SOURCE_DOCUMENT → CAS` 链生成 revision。未核对的剧本项阻止可见的“完成本来源审阅”动作。`projectSourceReviewWorkspace` 将已验证 revision 的受限文字同步投影到审阅清单、Markdown 文档和结论内依据，但不改写冻结原文、trace 或跨源结论；没有剧本映射的条目不显示虚假的编辑入口。数据库来源先将真实 DDL 解析为字段结构表，再提供完整结构和带行号的原始 DDL 复核。所有业务操作都是显式 `COMMAND`、`NAVIGATE` 或 `EXPAND`，每个页面或移动层最多一个主按钮：有待核对剧本时为“核对 N 项建议”，否则为“完成{来源}审阅”。工作台的“运行来源”只展示已锁定的资料实例，不能扫描、编辑、冻结或导入来源。
...
```text
审阅清单：来源材料 → 可核对 Claim → 剧本化修改
Markdown 文档：由当前审阅 revision 投影的只读正文
结论内依据：来源材料 → 审阅结论 → Markdown 段落
```

每条非元信息 Claim 持有完整 `evidenceRefs` 集合，而不是“第一条依据”。未决定的剧本化核对项只属于待核对清单，不重复计入已确认的审阅结论。阅读区把中文结论放在上方，SQL／DDL／代码／文档摘录在其“查看来源依据”折叠区显示；文档说明和业务目标只关联快照或审阅说明，不能错误指向索引、外键或 DML。这样读者可以从任一结论回看所有依据，而不把匿名代码块误读为另一条结论。
...
这些投影保持来源纯度。跨来源关系仍由正式运行状态机根据已准入来源动态投影到“本次读取发现”；内容 sidecar
