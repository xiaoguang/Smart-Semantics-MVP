...
前端消费冻结 Bundle 与当前审阅 revision。来源文档固定为“审阅事项／审阅结论／标准化文档”三页签，精确设计见[管伊佳数据标准化审阅体验设计](../design/data-standardization-review-experience.md)：审阅事项以 Claim 和运行级已准入比较为主数据，待确认事项标题下的“本来源建议”和“来源差异与比较”为两个同级 Box，另有逐项待补充资料和已处理事项；审阅结论只展示核心结论与默认展开的完整对象目录；标准化文档保持同一 revision 的 V6 九章阅读版和只读 Markdown 源文。每个结论内可展开
