...
| Maven PMD | 3.28.0 | 源码层 narrow rule set：`Throwable`、空 catch、`finally` return、危险 API，以及方法100/类1000以上的严重复杂度回归；typed boundary exception normalization 与资源/null bytecode分析交给SpotBugs。显式`jdkToolchain.version=17`，使PMD type resolver读取JDK 17类库而非启动Maven的shell JDK | `-Pquality` | PR/分析步骤 gate | 中/重，串行 | APPROVED CORE — IN-SCOPE EXECUTION DEFAULT AUTHORIZED |
...
2. PMD 独占少量源码危险构造与严重复杂度回归：`Throwable`、空 catch、`finally` return、危险 API，以及方法100/类1000以上的复杂度。`config/pmd-rules.xml` 只启用这些 named rules，不启用全量默认 ruleset、文件级基线排除或大规模 suppress；正常的 typed boundary exception wrapping 与 bytecode-level resource/null detection 不由 PMD 重复判断。
