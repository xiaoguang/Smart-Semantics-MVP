...
后续授权的宿主会话诊断确认根因是普通 Java/Maven 沙箱不能让 Codex 更新本机登录会话状态。它保持
客户仓库未执行、模型 sandbox 为只读，并创建新候选；该候选在 111.604 秒内完成一份 DRAFT 和一份
REVIEW，获得四项 `ANALYZED` 局部活动。它证明这一固定材料可产生受限、可读的活动解释，不扩大为
真实过程、报告或整仓验收。
