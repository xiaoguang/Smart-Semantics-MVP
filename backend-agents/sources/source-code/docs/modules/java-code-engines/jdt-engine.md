...
已批准、待实施的[模型批次解耦](../model-job-execution.md#7-固定材料与独立模型批次已批准待实施)在已保存M10之后开始，完全不打开本引擎。模型失败不清空JDT缓存或索引、不触发再扫；本页语法/导航失败合同保持不变。针对某个JDT超时的具名单入口复核是独立操作，其新结果不得就地覆盖旧材料或混入旧来源编号。
