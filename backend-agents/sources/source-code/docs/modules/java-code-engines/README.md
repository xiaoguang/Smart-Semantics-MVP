...
模型执行的失败不属于引擎重扫条件。已批准、待实施的[固定材料＋独立模型批次](../model-job-execution.md#7-固定材料与独立模型批次已批准待实施)在 M10 之后复用材料，JDT/JavaParser 均不打开会话；此处已有会话内 query 缓存、方法共享、索引格式不变。本次不增加新的引擎能力或插件接口。
