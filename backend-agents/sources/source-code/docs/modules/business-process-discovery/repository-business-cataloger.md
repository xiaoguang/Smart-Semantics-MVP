...
已实现稳定分片、并行两轮、唯一merge和完整覆盖。卡片已携带businessRules；同候选按`(activityId, variant)`保留多个业务用法；声明PROCESS_MEMBER却没有候选成员关系时会明确失败，不再自动降级。旧版真实14候选偏技术维护分类；新版v2目录仍必须由模型自行发现领域和用法，只有真实运行才能判断业务发现质量。
