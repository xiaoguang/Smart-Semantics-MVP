...
> 模型批次解耦（已批准、待实施）：全入口分母属于固定技术来源，不按模型批次重新发现。模型新批次复用材料中的入口处置；模型漏解释不改写为 Spring 或导航缺口。唯一执行合同见[模型执行 §7](../modules/model-job-execution.md#7-固定材料与独立模型批次已批准待实施)。本次不修改本步骤算法或产物。
