...
| **部分实现（M2 有界调用图）** | `CallGraphBuilder`已对冻结 fixture 产生唯一 Controller→Service、Service→Mapper、Mapper Java→XML statement 以及 call/return edges；重载 handler 或受显式 import 影响的 receiver 都记录 Gap，绝不按源码顺序或简单名称猜 target。`CallGraphExecution`会先以同一 reopened inputs 重开 M1，再构建并由`CallGraphModulePublisher`将调用图写为独立 M2 receipt-last artifact；后者把已重开 M1 payload 加入八项上游 lineage。但是，当前 builder 虽然为直接 Java 调用的 AST span 生成`ProvenanceDraftV1`，并把对应 ID 写入`CALL_SITE`节点和调用边的`evidenceDraftRefs`，却没有把这些 provenance entries 汇入`CallGraphDraft.provenanceDrafts`；draft 与 publisher 也没有拒绝这种“存在引用、registry 中没有条目”的断链。因此当前 M2 只能算调用关系切片，尚不是满足证据引用闭包的正式调用图。完整 receipt mutation matrix与完整仓库验收也仍未实现。 |
| **部分实现（M2→M3 可信重开）** | `PersistedCallGraphReader`和sealed `ReopenedCallGraph`已由真实 canonical M1/M2 modules 验证 address、type/schema、八项 upstream、controls、producer/completion、payload、profile与同一 M1/source/discovery basis；这些身份检查不等于 M2 provenance 闭包已经成立。上行所述断链修复并经 fresh-reopen mutation 验证以前，M3 不得把该 persisted M2 draft 当作证据闭合的调用图输入。控制流 builder 及 M3 module publication仍未实现。 |
...
| **下一实现门** | 先让`CallGraphBuilder`把每个生成的`ProvenanceDraftV1`恰好一次收入`CallGraphDraft.provenanceDrafts`，再由 draft、publisher 与 fresh reader 共同拒绝 unknown evidence ref、重复 provenance ID 和无人引用的 registry entry；该闭包门禁通过后，才完成 M2 的其余 fail-closed mutation matrix和 Mapper binding accounting。之后按本章完成 M3–M5，并以 M6 原子发布五图、index、Gap 和 receipt。删除任一源码关系时对应 edge 必须消失或形成 Gap。 |
