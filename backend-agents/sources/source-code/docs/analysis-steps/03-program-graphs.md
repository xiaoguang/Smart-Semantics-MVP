...
| **已实现（M6 有界正式发布）** | M6只消费fresh-reopened M1--M5与两个上游analysis-step publications，已能原子安装五张public graph、`graph-index.json`、`graph-gaps.jsonl`并由analysis-step store最后创建`program-graphs-receipt.json`。当前public wire已固定为data flow v2、evidence v3和graph index v2；旧data flow v1、evidence v2、index v1及M5 draft v2由canonical readers/registry拒绝，不提供兼容翻译。public graph已去除draft-only字段并闭合Evidence/catalog；M1--M4 local Gap一对一进入JSONL，scope Gap只进入coverages/index/receipts。历史有界基线覆盖exact-seven/exact-eight、public identity和当时的Gap分账，新增版本门由单独直接selectors证明；完整缺/多/交换/乱序/partial-install/collision mutation矩阵仍未关闭。 |
...
| **下一实现门** | P1的步骤内组合seam已关闭；全局run核心接线留给`RepositoryAnalysisAgent`实现批次。ProgramGraphs内部继续按补充backlog的P2--P8补齐M1--M6能力与多入口/完整jshERP离线验收。每个slice只关闭一个已列缺口；不得因为局部不支持重新推导八步主线。 |
