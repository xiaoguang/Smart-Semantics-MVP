...
状态：核心代码及最新 CHECK-only 导航精简已通过直接 RED/GREEN与干净本地质量构建。租户样本完成零调用、单编号的明确派生导入；无提示全仓全部阅读检查完成，详细过程生成中。A实际输入对照通过，真实检查尚未调用。真实实验及最终交付尚未完成，不以启动或程序通过代替业务质量通过。
...
| 实验 | 状态 | 输入隔离 |
| --- | --- | --- |
| A：有提示 2–3 个候选小样 | be508810...选材一次成功27候选。新版已验证JAR的零调用预览`analysis-run:05cb1f5d218009e4cdac5b4bf52da72ecd639084c945d40420df529afed5d52d`与旧预览逐项对照：仅三项已批准CHECK导航变化，3份补读前DRAFT输入及全部Schema逐字节不变；完整已读/未读导航分别19/307、14/312、13/313，均覆盖326。CHECK输入加Schema估计为222925/236362、226697/238220、212220/222722 token（o200k/cl100k），未包含额外封装/输出空间，非服务端精确计数。尚未调用A CHECK | 同一原始目录、326 Activity、冻结文本；不继承 B；全局選材及最终过程输入不变 |
| B：无提示全仓 | 用户授权的零调用导入4bbbac... FINISHED，原raw/FAILED不变。正式全仓`analysis-run:2af66afc4c3156b25e4a758fc0be47e0c909ac1f37640bdb400de6f89320f636`显式复用其全局选择及两个检查/过程样本；全部27个阅读检查阶段完成（25次新调用、2项复用），完整补读后正并行DRAFT/REVIEW，未归并/发布 | focus为空；原始目录和 Activity，不继承A；使用保留的已验证JAR dd52...匹配已有CHECK及pair，不绕过指纹 |
