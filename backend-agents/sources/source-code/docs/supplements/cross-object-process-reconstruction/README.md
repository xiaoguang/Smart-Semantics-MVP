...
状态：**2026-09-16 已批准的目标设计，尚未实施。** 本次交付仅为设计文档；产品内容生成 `none`。没有运行 JDT、Activity、过程模型或九章，也没有修改 Java、测试、Schema、生产 Prompt 资源或历史结果。
...
准确现状是：**完整 Activity 和少量源码预览先进入 DRAFT，选中的完整源码随后进入 REVIEW。** 不是“现在只有摘要”，也不是“326 个活动需要重建”。当前 FrozenAnalysisCorpus 还没有开放冻结文件搜索；设计复用底层已有 `VerifiedSourceTextReader`，新增的是 Step07 取材接线。
...
后续实施计划以本目录和[修改清单](../../plans/business-process-discovery-and-reconstruction-change-design.md)为准。未开始实施或真实生成；这些文档不授权重新生成 Activity。若将来确实需要重新生成 Activity，必须先说明原因及替代方案并获得用户明确同意。

## 7. 本次文档核对

已对照当前模块输入、CLI/运行记录、目录与来源读写，完成只读合同审查；三个推演中的已核实事实与未验证预期分别标明。共更新18份活跃文档、新增本目录5份详设；107处本地文件/章节链接检查通过，`git diff --check`通过。没有运行构建、测试、JDT或产品模型，也没有提交或推送。

受保护的`more-findings.md`保持字节不变，SHA256为`59b8381e7e81e3105ed6c6a8d93ce1dbea0247735bf1e6227d8f842b0d1d7f8e`。本任务临时progress的结论已收纳到本目录，不作为长期运行记录保留。真实自主发现效果、阅读容量及模型耗时仍按acceptance中的未验证项交给后续实施验收，不能以本次文档通过替代。
