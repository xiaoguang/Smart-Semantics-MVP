...
状态：**2026-09-16 更新为系统认识、聚焦选材、事实推理、业务写作和最终规则核对的详细设计。** 用户已接受最新采购正文的可读性；两个规则错误保留为准确性问题。生产已实现跨候选取材和两轮过程，本次三阶段尚待接入。见[实施状态](implementation-status.md)和[历史实测](delivery.md)。本轮只到三个例子的最小验收，之后再讨论全仓构建。
...
总体、Step07、直接受影响的业务发现模块、模型执行、中文 Prompt 导航、变更清单及 scoped AGENTS 同步目标/当前状态。逐步使用与三例执行约定集中在本补充，不把尚未实现的行为写成已经可用的CLI命令。Step01–06 的算法和格式不变；Step08 仍仅保留历史读取/渲染，不恢复旧生成器。
