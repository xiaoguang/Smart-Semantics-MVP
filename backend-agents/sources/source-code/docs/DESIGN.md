...
| 分析步骤“已证明代码事实” | **部分实现（候选清单）** | M1会从fresh-reopened完整五图与应用发现输入建立、安装并重开`FactCandidateSet`；它检查entry、调用目标、参数、控制和当前收集到的Evidence闭合，并对缺失组合保留处置。 | 当前M1内存投影尚未保留完整source/rule record，且尚未要求call-site/local-origin evidence，不能交给M2作为逐atom Proof 输入；修正后仍须实现M2 CLOSED Proof与admitted `CodeFact`、M3 Fact/Proof/Gap/accounting五项正式输出。历史POC发现的Evidence语义错配只作为反例，不能成为当前Fact。 |
