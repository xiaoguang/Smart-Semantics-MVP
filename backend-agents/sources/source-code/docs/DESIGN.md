...
| Canonical bytes、身份原语与artifact policy registry | **部分实现（源码清单模块持久化纵切）** | `CanonicalJsonCodec`、不可变bytes、typed identity/address与path-free `CanonicalArtifactPolicyRegistry`已能重验canonical JSON、policy self-excluded ID和完整文档SHA；`CanonicalModuleArtifactStore`能对源码清单M1/M2的`MODULE_ARTIFACT_JSON`与M3恰好三项`STANDALONE_JSON`/`CANONICAL_JSONL` payload做原子receipt-last安装、fresh reopen、descriptor/root/receipt ID+SHA重算，拒绝策略表不一致、符号链接、空/缺receipt、非等价碰撞、错误文件组与非canonical输入顺序。 | 这仍不是完整module store：RAW UTF-8、其他分析模块payload、analysis-step/run store、生产root bootstrap和runtime未实现；`PATH_FREE_COMPLETE_UTF8`的具体安全type/schema allowlist也尚未发布，因此当前registry不猜测该目录。 |
| Local Git capture / 分析步骤“已验证源码清单” | **部分实现（独立capture纵切与共享持久化预备）** | `LocalGitCommitCaptureAdapter`已在synthetic local Git repository上以exact commit、raw Git objects、text/media/100755 inventory和path-free registration验证一条私有快照安装链；symlink拒绝及工作区独立性已有定向测试。M1/M2/M3业务算法、private source registry lookup、analysis-step store及四项reader-visible正式输出仍不存在；没有任何jshERP capture或分析结果。共享store只保存已注册模块payload形状，不能替代本步骤。 | 实现M1→M3和analysis-step publish，再对完整固定commit做离线验收。 |
