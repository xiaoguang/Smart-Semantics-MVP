...
| Canonical bytes、身份原语与artifact policy registry | **部分实现（foundation）** | `CanonicalJsonCodec`、不可变bytes、typed identity/address与path-free `CanonicalArtifactPolicyRegistry`已能重验canonical JSON、policy self-excluded ID和完整文档SHA；policy registry是公开interface，失败码使用已发布闭集。 | 三个filesystem store、receipt-last安装、schema/type catalogue、JSONL/RAW UTF-8写入、run manifest和runtime仍未实现；`PATH_FREE_COMPLETE_UTF8`的具体安全type/schema allowlist也尚未发布，因此当前registry不猜测该目录。 |
