...
| 分析步骤“已证明代码事实” | **部分实现（候选清单）** | M1会从fresh-reopened完整五图与应用发现输入建立、安装并重开`FactCandidateSet`；它精确检查entry、调用目标、参数、控制和Evidence闭合，并对缺失组合保留处置。 | M2尚未为候选建立CLOSED Proof或admit `CodeFact`，M3尚未发布Fact/Proof/Gap/accounting五项正式输出；历史POC发现的Evidence语义错配只作为反例，不能成为当前Fact。 |
