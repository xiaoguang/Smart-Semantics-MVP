...
| Canonical bytes、身份原语与artifact policy registry | **部分实现（首个落盘纵切）** | `CanonicalJsonCodec`、不可变bytes、typed identity/address与path-free `CanonicalArtifactPolicyRegistry`已能重验canonical JSON、policy self-excluded ID和完整文档SHA；另有`CanonicalModuleArtifactStore`支持源码盘点 M1 的唯一已注册`MODULE_ARTIFACT_JSON` envelope：原子receipt-last安装、fresh reopen、descriptor/root/receipt ID+SHA重算，并拒绝策略表不一致、符号链接、空/缺 receipt与非等价碰撞。 | 这只是一个M1 JSON vertical slice，不是完整module store：JSONL、RAW UTF-8、其他module payload、analysis-step/run store、生产root bootstrap和runtime仍未实现；`PATH_FREE_COMPLETE_UTF8`的具体安全type/schema allowlist也尚未发布，因此当前registry不猜测该目录。 |
