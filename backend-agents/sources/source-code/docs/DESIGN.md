...
| Local Git capture / 分析步骤“已验证源码清单” | **尚未实现（共享持久化预备已完成）** | `capture.localgit`与`analysis.inventory`仍只有package骨架；M1/M2/M3业务算法、capture、source registration、analysis-step store和四项真正reader-visible正式输出均不存在。共享store可保存这些模块规定的payload形状，但没有生成或语义验证它们。 | 需按分析步骤文档实现业务模块并对完整固定commit做离线验收。 |
