...
public record FlowModelTaskSet(
  ...
  public FlowModelTaskSet {
    ...
    tasks = tasks.stream().sorted(Comparator.comparing(FlowModelTask::flowSliceId).thenComparing(FlowModelTask::round)).toList();
    if (tasks.size() != eligibleR1R2FlowSliceIds.size() * 2
        ...
        || !eligibleR1R2FlowSliceIds.equals(
            tasks.stream().filter(value -> "R1".equals(value.round())).map(FlowModelTask::flowSliceId).toList())
        || !eligibleR1R2FlowSliceIds.equals(
            tasks.stream().filter(value -> "R2".equals(value.round())).map(FlowModelTask::flowSliceId).toList())) {
  ...
  private static List<FlowModelTaskShardReceipt> shards(
    ...
    List<FlowModelTaskShardReceipt> ordered = receipts.stream().sorted(Comparator.comparing(FlowModelTaskShardReceipt::shardId)).toList();
    List<String> actualFlows = ordered.stream().flatMap(value -> value.denominatorFlowSliceIds().stream()).sorted().toList();
    List<String> actualTasks = ordered.stream().flatMap(value -> value.outputTaskSpecIds().stream()).sorted().toList();
    List<String> expectedTasks = tasks.stream().filter(value -> round.equals(value.round())).map(FlowModelTask::taskSpecId).sorted().toList();
