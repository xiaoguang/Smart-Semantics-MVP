...
Stage05已经证明“代码发生什么”，但新客户仓库的业务词、业务主张和待确认问题不可能预先穷举。Stage06只做两个受控动作：每条可安全交给模型的Flow先独立执行一次`R0_REGISTRY_PROPOSAL`，程序验证proposal并冻结全仓唯一`RepositoryInterpretationRegistry`；随后同一Flow的R1/R2只能选择该registry的finite provisional keys。Stage07仍由程序决定是否准入。

数量合同固定：

- `N`：Stage05全部Flow/Capsule；
- `E`：其中`modelEligibility=ELIGIBLE`的Flow；
- `I=N-E`：model-ineligible Flow；
- `R`：`E`条R0 disposition中`READY_FOR_FREEZE`的Flow。

Stage06精确规划`E`个R0 task和`2R`个R1/R2 task，总数始终为`E+2R`。成功发布的Stage06中，每个planned task恰有一个`ModelTaskDispositionV1`。R2若因同Flow R1返回typed GAP/FAILED而不调用Provider，仍保留R2 task并写`NOT_RUN_UPSTREAM_FAILED` disposition。Provider调用一旦开始却没有得到可验证响应，当前run直接`FAILED`，不自动重试、不切换Provider，也不把不确定状态伪装成业务Gap。

ineligible Flow在Stage06的task、round、candidate和disposition均为零；Stage07根据其Stage05 Gap生成唯一technical fallback。每次模型调用只见一条Flow的同一EvidenceCapsule；单Flow成功不能结束阶段或run。

## 2. 具体输入与 DepotHead 例子

> **示例分类：NARRATIVE_ILLUSTRATION / TARGET_ILLUSTRATIVE_NOT_CURRENT_OUTPUT。** 下文用未来已闭合的DepotHead Flow展示目标流程；story IDs不是wire或golden。当前fixed slice仍是0 Flow、0 Capsule、0 R0/R1/R2 task/call。
...
1. fresh-reopen Stage05的五项semantic artifacts与receipt，重验完整`N`条Flow/Capsule双射、`N=E+I`、eligible/ineligible不交叠并集、逐ineligible Flow非空Gap mapping，以及全部Fact/Proof/Outcome/span/projection refs。
...
8. M6重算`N/E/I/R`、R0/R1/R2 shard union、`E+2R` task/disposition双射、proposal/candidate/Flow处置集合和全部refs，先安装九个semantic payload与module receipt。Stage store fresh-reopen M6后安装同九项并最后写stage receipt。reader-visible Stage06固定为九项semantic + receipt，共十个文件。
...
| `stage-receipt.json` | 九项semantic descriptors、Stage05 publication ref、controls与M6 provenance |

精确集合关系以排序ID数组保存和校验，不能只比较count：

~~~text
stage05FlowSliceIds = modelEligibleFlowSliceIds ⊎ modelIneligibleFlowSliceIds
...
modelIneligibleFlowSliceIds ∩ everyStage06PerFlowDomain = ∅
~~~
...
Provider started-call数为`E + R + count(R1 RESPONSE_ACCEPTED)`；只有所有R1都accepted时等于`E+2R`，再加`R=E`才等于`3E`。业务typed GAP/FAILED可减少R2调用，但不能删除planned R2 task或disposition。Stage06不生成Markdown。

## 5. 下游怎样消费而不返工

Stage07只读取Stage04 Facts、Stage05 Flows/Capsules、Stage06九项semantic artifacts及各自receipt，验证`registryProposalId → provisionalKey → interpretationProposalId → selectedKey`后决定`meaningId`。它不调用Provider、不回读源码、不读取runtime journal，也不从seed或label猜Fact。

如果原Stage07执行失败，调用者可通过`executeStage(targetStage=7)`创建新run，显式传入validated Stage01–06 publications。新执行fresh-reopen Stage06十文件和Stage07所需Stage04/05 inputs，只运行Stage07；Stage01–06不运行，Provider调用为零。这是业务artifact复用，不是同一run恢复。

| Stage07开始前必须成立 | Stage06成功后保证 |
| --- | --- |
| Stage05完整`N`条Flow/Capsule双射与`ELIGIBLE ⊎ INELIGIBLE`分区 | Stage06只消费`E`条eligible Flow；三轮源码证据均来自对应Capsule |
| 九项semantic + receipt、六个module publications完整 | 每eligible Flow有R0及最终disposition；ineligible Flow没有Stage06 per-Flow对象；恰一registry |
...
- **成功**：`N/E/I/R`、registry、九项semantic、六个module publications、task/disposition双射和Stage receipt全部闭合；`E=0`合法，仍产生0-item registry和十文件。
...
- **run失败**：Provider调用开始后transport/runtime中断、无可验证response、configured/observed runtime不符，或Provider前的task/session/policy无法执行。当前run为`FAILED`，不自动重试或切换；已安装Stage01–05及Stage06前驱module artifacts/诊断保留，但没有伪Stage06 success publication。
...
- **显式复用**：Stage07或后续stage的新执行可消费完整Stage06 publication；若调用者明确从Stage05新开Stage06，则那是新的模型工作和新run，不继承旧Provider调用状态。
...
顺序固定为`RegistryProposalTaskCompiler → RegistryProposalRunner → RepositoryInterpretationRegistryFreezer → FiniteKeyFlowTaskCompiler → InterpretationRunner → InterpretationPublicationSpecifier → CanonicalStageArtifactStore`。只有M2/M5持有Provider Interface；每个task最多调用一次。持久化module keys仍固定为`registry-task-compiler / registry-proposal-runner / registry-freezer / flow-task-compiler / interpretation-runner / publish`。

| 模块 | exact direct upstream | exact输出与失败 | 下游保证 |
| --- | --- | --- | --- |
| M1 RegistryProposalTaskCompiler | Stage05五项semantic refs、Stage05 receipt、run-request、prompt/schema/budget refs | `stage06-registry-proposal-task-set-v2`：`E` tasks、完整input JSON、R0 shard denominator=`E`；分区/ref/hash错则run失败 | M2无需读取Stage05或内存draft |
| M2 RegistryProposalRunner | M1 task-set ref、冻结Provider/runtime policy | `stage06-registry-proposal-execution-set-v3`：`E` rounds/receipts/dispositions及validated proposals；Provider每task一次，transport/runtime失败使run失败 | M3只读canonical R0业务结果 |
| M3 RepositoryInterpretationRegistryFreezer | M1、M2、Stage05五项refs | `stage06-repository-interpretation-registry-v2`：唯一registry、`E` flow dispositions、proposal accounting；缺/重/碰撞fatal | M4获得finite same-Flow keys |
| M4 FiniteKeyFlowTaskCompiler | M3、Stage05五项、run-request、R1/R2 prompt/schema/budget refs | `stage06-flow-task-set-v4`：`2R` tasks；R1和R2 shard denominator各=`R` | M5无需决定allowlist/session/input |
| M5 InterpretationRunner | M2、M3、M4 refs、冻结Provider/runtime policy | `stage06-model-execution-set-v5`：actual rounds/receipts/proposals/candidates、`E` final dispositions和`2R` task dispositions；Provider失败使run失败 | M6无需Provider或runtime state |
| M6 InterpretationPublicationSpecifier | M1–M5 refs | 一次module install恰九项public semantic bytes；closure/canonical/collision错误fatal；不得含stage root/receipt | Stage store可原样安装九项并最后写receipt |

DepotHead walkthrough在每个模块的投影固定为：M1一个完整Capsule R0 task；M2一个有basis的“批量审核或反审核”proposal/处置；M3一个`TERM_P_<hex64>` registry item；M4同Flow的R1/R2两个finite-key tasks；M5一个KEEP candidate或typed Gap；M6发布九项semantic并由Stage store补第十个receipt。若DepotHead保持当前真实0-Capsule Gap，这六个per-Flow投影都为零，但0-item registry和十文件仍存在。

每个模块都必须满足以下实现brief：

- **M1测试/实现**：public seam `compileRegistryProposalTasks(stage05, requestV2)`；Luna selector `Stage06RegistryProposalTaskCompilerTest`覆盖`N=0,E=0`、`N>0,E=0`、mixed eligibility、完整Capsule input、seed、partition/shard/hash反例。Terra只改`target/stage06/registry-task-compiler/`。
- **M2测试/实现**：public seam `runRegistryProposals(taskSet, provider)`；Luna selector `Stage06RegistryProposalRunnerTest`覆盖0call、valid term、typed GAP/FAILED、basis/Unicode/seed反例、single-call、transport failure no retry、双Flow隔离。Terra只改`target/stage06/registry-proposal-runner/`。
- **M3测试/实现**：public seam `freeze(taskSet, executionSet, stage05)`；Luna selector `Stage06RepositoryInterpretationRegistryFreezerTest`覆盖empty、one item、same-label two-flow、seed lineage、shuffle、missing/duplicate/collision、fresh reopen。Terra只改`target/stage06/registry-freezer/`。
- **M4测试/实现**：public seam `compileFiniteKeyTasks(stage05, registry, requestV2)`；Luna selector `Stage06FiniteKeyFlowTaskCompilerTest`覆盖0ready、single、multi-flow key isolation、READY missing key、`R/R` shard denominators和input hashes。Terra只改`target/stage06/flow-task-compiler/`。
- **M5测试/实现**：public seam `runInterpretations(taskSet, registry, r0Dispositions, provider)`；Luna selector `Stage06InterpretationRunnerTest`覆盖R1/R2 success、R1 typed GAP/FAILED→R2 NOT_RUN、unknown/cross-flow key、R2 expansion、transport failure no retry、双Flow隔离。Terra只改`target/stage06/interpretation-runner/`。
- **M6测试/实现**：public seam `specify(m1,m2,m3,m4,m5,controls)`；Luna selector `Stage06InterpretationPublicationSpecifierTest`覆盖`E=0`十文件、`E=1,R=1`、`E=2,R=1`、`E=2,R=2`、`E/R/R` shards、task/disposition ID-set equality、ineligible leakage、partial-install/collision/fresh-reopen。Terra只改`target/stage06/publish/`。

所有Luna tests使用scripted provider，禁止live Provider、网络、客户Maven或mock canonical/accounting。Terra观察对应RED后只实现最小GREEN；任何field/model/failure/cross-stage变化按DESIGN 13.11 STOP交Sol/ultra。

### 8.0.1 模块 artifact wire schemas

M1–M5使用DESIGN 13.3 `ModuleArtifact<T>` envelope；M6直接安装九个registered stage bytes。unknown field/version拒绝。

| module artifact / schema | exact payload与排序 |
| --- | --- |
| `modules/01-registry-task-compiler/registry-proposal-task-set.json` / `stage06-registry-proposal-task-set-v2` | `taskSetId,stage05PublicationRef,flowCount=E,eligibleFlowSliceIds,tasks[E],taskShardReceipts,runtimePolicy,organizationRegistrySeedRef`；tasks按flowSliceId，每个含完整`RegistryProposalInputV1`及input/schema/prompt/runtime hashes |
| `modules/02-registry-proposal-runner/registry-proposal-execution-set.json` / `stage06-registry-proposal-execution-set-v3` | `executionSetId,taskSetId,rounds[E],receipts[E],validatedProposals,flowDispositions[E],shardReceipts,callCount=E`；round/receipt/disposition按flow，proposal按proposalId |
| `modules/03-registry-freezer/repository-interpretation-registry.json` / `stage06-repository-interpretation-registry-v2` | `repositoryInterpretationRegistryId,eligibleFlowSliceIds[E],items,flowDispositions[E],proposalAccounting,organizationRegistrySeedRef,closed=true`；items按proposalKind/flow/provisionalKey |
| `modules/04-flow-task-compiler/flow-task-set.json` / `stage06-flow-task-set-v4` | `flowTaskSetId,repositoryInterpretationRegistryId,eligibleR1R2FlowSliceIds[R],tasks[2R],r1ShardReceipts,r2ShardReceipts,runtimePolicy`；tasks按flow后R1/R2，两个shard union分别等于R个Flow IDs |
| `modules/05-interpretation-runner/model-execution-set.json` / `stage06-model-execution-set-v5` | `executionSetId,flowTaskSetId,repositoryInterpretationRegistryId,rounds,receipts,interpretationProposals,candidates,flowDispositions[E],r1ShardReceipts,r2ShardReceipts,callCounts`；round/receipt按task，candidate/disposition按flow |
...
| filename | schema |
| --- | --- |
| `registry-proposal-tasks.jsonl` | `stage06-registry-proposal-task-v2` |
| `registry-proposal-rounds.jsonl` | `stage06-registry-proposal-round-v2` |
| `registry-proposal-dispositions.jsonl` | `stage06-registry-proposal-disposition-v2` |
| `repository-interpretation-registry.json` | `stage06-repository-interpretation-registry-v2` |
| `flow-model-tasks.jsonl` | `stage06-flow-model-task-v3` |
| `model-rounds.jsonl` | `stage06-model-round-v2` |
| `generation-receipts.jsonl` | `stage06-generation-receipt-v2` |
| `interpretation-candidates.jsonl` | `stage06-interpretation-candidate-v2` |
| `flow-interpretation-dispositions.jsonl` | `stage06-flow-interpretation-disposition-v2` |
...
`capsuleView`是对应Stage05 Capsule的完整封闭投影：fact atoms及Proof IDs、Gap reason/evidence、Outcome guard/terminal、连续source spans、projection obligations和R0 basis allowlist逐字段复制。R0/R1/R2同Flow都使用相同view；`allowedRegistryItems`按provisionalKey排序。发送bytes恰为`canonicalJson(inputJson)`，runner发送前重算SHA。R2的same-session target必须显式绑定R1 task和round，不能靠Provider“latest session”。
...
- runner在调用前验证task/request/runtime identity；调用开始后若transport中断、进程结束、响应缺失或observed runtime不符，当前run`FAILED`并保留安全诊断。该失败不进入`ModelTaskDispositionV1`，也不安装Stage06 success set。
- 只有完整收到并通过版本化response envelope校验的响应，才能写`ModelRound`、`GenerationReceipt`和task disposition。
- 已完整安装的M1/M3/M4等业务module artifacts继续可inspect；调用者可显式从Stage05创建新的Stage06 execution，但系统不把它当旧调用续接，也不提供exactly-once保证。
...
- Provider transport/runtime failure使run FAILED，started call count为1且retry/switch count为0；不得产生Stage06 success publication。
...
- 改变shard size或程序遍历顺序不改变registry和public Stage06 bytes。

只有`E`个R0 dispositions、一个registry、`E`个final dispositions、`2R`个R1/R2 tasks以及ineligible零Stage06 artifact约束同时闭合，Stage06才可交付。

### 8.7 已冻结裁决：实现者不得自由推断

- 仍只有Stage06可调用LLM；R0是内部模块，不是第九阶段。
- Stage05 EvidenceCapsule是R0/R1/R2唯一源码语义来源；organization seed仅optional hint。
...
- 任何field、key identity、failure、model boundary或cross-stage lineage变化必须按DESIGN 13.11 STOP并交Sol/ultra Design Authority；跨阶段/业务目标由用户确认。
## 9. 当前实现差距审计

| 状态 | 当前事实 |
| --- | --- |
| **部分具备（scripted R1/R2）** | 现有Stage03/04有perFlow finite-key R1/R2、runtime identity和bounded scripted tests |
| **R0/freeze缺失** | 当前没有RegistryProposalTask、bounded open proposal validator、全仓RepositoryInterpretationRegistry freezer或proposal→provisionalKey lineage；现有两轮直连主线不符合`E+2R` task/disposition合同 |
