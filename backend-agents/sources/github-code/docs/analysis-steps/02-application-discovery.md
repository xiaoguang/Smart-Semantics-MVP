...
| 状态 | 当前事实 |
| --- | --- |
| **已实现（M1 持久化画像草稿）** | 当前`com.linguan.codemd.target.stage02.applicationprofile`从真实旧Stage01 publication重开三份语义文件；`Stage02ApplicationProfileDetectorTest`覆盖Java/Spring MVC/MyBatis signals、release conflict、不同store root和媒体分母。持久化schema仍是`stage02-application-profile-draft-v2`。 |
| **已实现（M2 持久化入口发现）** | `SpringHttpEntryDiscoverer`重开旧M1/Stage01 publications，仅对inventory内冻结Java bytes使用JavaParser；双Controller fixture、dynamic-route Gap和重复annotation locator均有定向覆盖，schema仍是`stage02-http-entry-discovery-v2`。 |
| **已实现（M3 持久化 Mapper catalog）** | `MapperCapabilityCataloger`从旧M1/Stage01 artifacts读取Java/XML bytes，持久化candidate catalog；标准MyBatis DOCTYPE、XXE fatal和namespace Gap均有定向覆盖。 |
| **已实现（M4 五文件发布）** | 当前`Stage02PublicationSpecifier`fresh-reopen M1–M3并发布四个semantic files；旧Stage store最后写`stage-receipt.json`。双入口与零入口fixture通过直接selector。 |
| **仍未符合目标** | 上述是pre-reset小型complete-capture闭环：尚未改用`org.sourceanalysis.app.analysis.discovery`、新semantic schema/receipt，也未对完整固定jshERP commit运行或由新run-centric orchestrator连接ProgramGraphs。 |
| **真实样例边界** | 当前代码能看到 DepotHead route 的相关字节，但后续数据流/Fact/Flow closure 未通过；不得把 entry discovery 写成当前 DepotHead complete Flow |

目标分析步骤只固定入口和能力分母。调用、控制、数据、证据关系属于分析步骤“程序图”；pre-reset类不能被包装成兼容层。
