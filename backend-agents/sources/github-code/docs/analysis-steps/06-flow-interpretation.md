...
DepotHead walkthrough在每个模块的投影固定为：M1一个完整Capsule R0 task；M2一个有basis的“批量审核或反审核”proposal/处置；M3一个`TERM_P_<hex64>` registry item；M4同Flow的R1/R2两个finite-key tasks；M5一个KEEP candidate或typed Gap；M6发布九项semantic并由AnalysisStep store补第十个receipt。若DepotHead保持当前真实0-Capsule Gap，这六个per-Flow投影都为零，但0-item registry和十文件仍存在。
...
| 状态 | 当前事实 |
| --- | --- |
| **部分具备（scripted R1/R2）** | 现有旧`stage03`/`stage04`路径有per-Flow finite-key R1/R2、runtime identity和bounded scripted tests。 |
...
| **真实DepotHead当前零调用** | pre-reset业务流程结果仍为0 Capsule，当前jshERP slice必须是0 R0/R1/R2 task/call；不能绕过BusinessFlows让模型读Service。 |
| **artifact缺失** | 没有独立run/steps/06十文件或六个module artifact sets；live Provider不在本次授权内 |
