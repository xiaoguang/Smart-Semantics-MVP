...
实现从完整 Wire Reset 开始。当前 `sources/github-code/`、
`com.linguan.codemd`、编号 package/type/schema、旧 receipt 和旧 run store
都是下文当前审计中的 pre-reset 事实，不是兼容合同。未来实现整体移动、
删除旧 production/test/fixture wire，只写新格式；不增加 alias、symlink、
dual reader/writer、迁移 bridge、fallback reader 或旧 artifact 转换。任何旧
...
因果关系必须保持：来源不固定就不能可信定位；**完整入口分母**不固定就不能证明仓库覆盖；五图不闭合就不能证明 Fact；Fact 不闭合就不能编译 Flow；没有 Flow/Capsule 就不得调用模型。当前 DepotHead 恰好停在 Fact/Flow closure Gap，因此它在分析步骤“流程解释”是零任务；同一仓库的其他eligible Flow仍独立执行，分析步骤“仓库知识”和“九章文档”把全部slice的Facts、解释、失败、排除和Gaps合成非空、可审阅的单一仓库知识与九章文档。
...
各AnalysisStep文档必须列出上述standalone schema的完整字段；同名module preimage schema和standalone output schema不是同一个artifact，不能互换。reader exact-version解析；旧artifact不可原位迁移。当前实现仍可能只支持本文件第15节审计列出的旧/有限产物，本表不得被引用为“代码已支持”。
...
| 目标分析步骤/横切能力 | 中文状态 | 当前 pre-reset 事实 | 与目标的差距 |
| --- | --- | --- | --- |
| Local Git capture / 分析步骤“已验证源码清单” | **部分实现（小型持久化闭环）** | 当前类仍在`com.linguan.codemd.target.stage01`，以`Stage01`类型和旧`stage-receipt.json`完成小型synthetic capture/request/index/publication测试。 | 未执行Wire Reset，未使用新package、semantic wire或receipt；尚未对完整固定jshERP commit执行端到端离线验收。 |
| 分析步骤“应用发现” | **部分实现（小型五文件闭环）** | 当前类仍在`com.linguan.codemd.target.stage02`；`Stage02ApplicationProfileDetectorTest`等selectors覆盖静态Java/Spring MVC/MyBatis发现，`Stage02PublicationSpecifier`写旧`stage-receipt.json`。 | 尚未运行完整固定jshERP commit，未使用新package/semantic schema/receipt，也没有新run-centric orchestrator连接后续分析步骤。 |
| 分析步骤“程序图” | **缺失目标产物** | 当前`RepositoryModel`、`Stage01FlowView`只有部分结构、调用和CFG投影。 | 没有五个standalone first-class graph files；通用data-flow/evidence graph不足。 |
| 分析步骤“已证明代码事实” | **有限profile已测；DepotHead仍有缺口** | 当前旧`Stage01` M3有`CodeFact`、`ProofPack`、`GapLedger`与accounting的in-memory bounded tests。 | Fact registry偏合成剖面；无法充分证明DepotHead status/ids跨层通用数据流，也没有新语义持久化出口。 |
| 分析步骤“业务流程” | **有限profile已测；真实样例为缺口** | 当前旧`Stage02Compiler`/`Stage02Result`在受支持fixture上工作。 | 固定jshERP slice的诚实结果仍是Gap、0 Flow、0 Capsule；结果主要在内存。 |
| 分析步骤“流程解释” | **部分具备（scripted R1/R2）** | 当前旧`stage03`/`stage04`路径有逐Flow R1/R2、有限key和bounded scripted tests。 | 没有R0 proposal/freezer、单一RepositoryInterpretationRegistry、`E+2R` accounting或独立semantic artifacts；0 Capsule意味着当前应有0 task/call。 |
| 分析步骤“仓库知识” | **部分具备（有限profile、内存后归档）** | 当前旧`stage03`路径有程序准入、technical fallback、有限merge与typed knowledge。 | 通用anchor/conflict/owner能力有限，没有目标语义步骤的独立持久化。 |
| 分析步骤“九章文档” | **部分具备（final-only Candidate）** | 当前`archive-v2`可保存最终Candidate、plan、trace、rounds、receipts并做零Provider validation；Candidate未发布。 | 只有最终Candidate归档，不是八个分析步骤逐段production persistence；目标final ledger、唯一九章与run manifest未完整实现。 |
| Java/CLI/HTTP runtime | **目标Interface缺失；旧路径待删除** | 当前旧`target` CLI/loopback HTTP有bounded tests，仍暴露`CodeToMarkdownAgent`与`executeStage`词汇。 | Wire Reset必须删除旧接口、CLI和兼容路径，按七方法Interface重新实现`executeStep`及semantic address；不保留迁移桥。 |
| 旧POC | **history-only** | 人工Manifest/LockedFact、旧baseline/archive只解释历史成熟度。 | 不得进入活动导航、目标主线或DepotHead成功证据。 |

因此当前最重要的结论是：

- pre-reset `Stage01`/`Stage02`有限能力通过不等于八个分析步骤生产主线已实现；
- final-only Candidate archive 不等于逐分析步骤可观察、可验证和可显式复用；
