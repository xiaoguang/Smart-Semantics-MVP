...
export function sha256HexSync(content: string | Uint8Array) {
...
export async function sha256Hex(content: string | Uint8Array, subtle: SubtleCrypto | null = globalThis.crypto?.subtle ?? null) {
  const bytes = typeof content === 'string' ? new TextEncoder().encode(content) : content;
