...
import { createOmnichannelFindings, omnichannelEvidenceLocators, omnichannelScenarioFixture, omnichannelSourceAssets } from './omnichannel-fixture.ts';
import type { StorageLike } from './runtime.ts';
import { WorkbenchError } from './runtime.ts';

type StoredState = {
  schemaVersion: 1;
  systemCode: 'omnichannel_retail_ops';
  lockVersion: number;
  batch: ModelingBatch;
...
function initial(): StoredState {
  return {
    schemaVersion: 1, systemCode: 'omnichannel_retail_ops', lockVersion: 0,
    batch: { batchId: 'omnichannel-batch-1-r1', systemCode: 'omnichannel_retail_ops', revision: 1, state: 'COLLECTING', sourceIds: [], fingerprint: '', findings: [] },
...
function hydrate(state: StoredState): WorkspaceSnapshot {
  ...
  const decisions = result ? Object.values(result.decisions) : [];
  const hydrated = result ? [{
    ...clone(result), fixture: document, totalItemCount: 120, decidedItemCount: decisions.length,
    ...
    pendingItemCount: 120 - decisions.length, allItemsDecided: decisions.length === 120,
  }] : [];
  return {
    schemaVersion: 4, systemCode: state.systemCode, lockVersion: state.lockVersion, uploads: [], unknownUploads: [],
    results: hydrated, catalogs: state.catalog ? [clone(state.catalog)] : [], currentCatalogVersion: state.catalog?.modelVersion ?? null,
    fixture: omnichannelScenarioFixture, nextExpectedVersion: null, recoveryRequired: false, recoveryMessage: null,
    sourceWorkspace: {
      availableSources: clone(omnichannelSourceAssets),
      selectedSources: omnichannelSourceAssets.filter((item) => state.batch.sourceIds.includes(item.sourceId)).map(clone),
      evidenceLocators: clone(omnichannelEvidenceLocators),
      batch: clone(state.batch),
      history: clone(state.batchHistory ?? []),
    },
  };
}
...
function automaticDecisions(now: string) {
  const items = document.review.items.filter((item) => item.reviewClass === 'AUTO');
  return Object.fromEntries(items.map((item) => [item.id, {
    ...
    reason: `置信度 ${Math.round((item.confidence ?? 0.97) * 100)}%，并由正式资料与结构快照相互验证`,
    confidence: item.confidence ?? 0.97, decidedAt: now,
  } satisfies ReviewDecision]));
}

export function createOmnichannelWorkbenchRuntime(options: {
  storage: StorageLike; now?: () => string;
}): WorkbenchRuntime {
  const storageKey = 'linguan:ai-modeling:omnichannel_retail_ops:sources:v1';
  ...
  const load = () => {
    try {
      ...
      if (parsed.schemaVersion !== 1 || parsed.systemCode !== 'omnichannel_retail_ops') return initial();
      parsed.batchHistory ??= [];
      return parsed;
  ...
  const requireSystem = (systemCode: string) => {
    if (systemCode !== 'omnichannel_retail_ops') throw new WorkbenchError('SYSTEM_NOT_FOUND', '模型空间不存在');
  };
...
function apply(state: StoredState, command: WorkbenchCommand, timestamp: string) {
  if (command.type === 'ADD_SOURCES') {
    const invalid = command.sourceIds.find((id) => !omnichannelSourceAssets.some((item) => item.sourceId === id));
    ...
    if (state.batch.state !== 'COLLECTING') {
      ...
      state.batch = { ...state.batch, batchId: `omnichannel-batch-1-r${state.batch.revision + 1}`, revision: state.batch.revision + 1, state: 'COLLECTING', sourceIds: [...state.batch.sourceIds], fingerprint: '', findings: [] };
      state.result = null;
    }
  ...
  if (command.type === 'REMOVE_SOURCE') {
    if (state.batch.state !== 'COLLECTING') throw new WorkbenchError('BATCH_FROZEN', '资料包已冻结，不能移除来源');
    state.batch.sourceIds = state.batch.sourceIds.filter((id) => id !== command.sourceId);
    return;
  }
  if (command.type === 'START_MODELING') {
    if (state.batch.sourceIds.length !== omnichannelSourceAssets.length) throw new WorkbenchError('SOURCE_COMBINATION_NO_RESULT', `当前来源组合尚无处理结果，请先补齐本空间的 ${omnichannelSourceAssets.length} 个预置来源`);
    state.batch.fingerprint = computeSourceBundleFingerprint(state.batch.sourceIds.map((id) => omnichannelSourceAssets.find((item) => item.sourceId === id)!.fingerprint));
    state.batch.findings = createOmnichannelFindings();
  ...
  if (command.type === 'GENERATE_MODEL') {
    const unresolved = state.batch.findings.filter((item) => item.severity === 'BLOCKER'
      && (!item.decision || item.decision.resolutionId === 'keep_blocked'));
    ...
    state.result = {
      documentVersion: 'v1', status: 'GENERATED', generatedAt: timestamp, decisions: automaticDecisions(timestamp),
      confirmedGroups: ['auto'], checks: [], publishedAt: null,
    };
  ...
  if (!state.result) throw new WorkbenchError('MODEL_NOT_GENERATED', '请先生成模型');
  requireReason(command.reviewer, command.reason);
  if (Object.keys(state.result.decisions).length !== 120) throw new WorkbenchError('REVIEW_INCOMPLETE', '请先确认全部人工审核项');
  state.result.status = 'PUBLISHED'; state.result.publishedAt = timestamp;
  state.result.checks = [
    { id: 'RELATION_ENDPOINTS', label: '表之间可以正确关联', passed: true, detail: '13 条关系均能定位到两端表和字段' },
    { id: 'FIELD_UNIQUENESS', label: '字段定义没有冲突', passed: true, detail: '10 张表的 72 个字段编码唯一' },
    { id: 'METRIC_DEPENDENCIES', label: '指标具备计算条件', passed: true, detail: '10 个指标依赖字段完整' },
    { id: 'SOURCE_EVIDENCE', label: '结果可以追溯设计资料', passed: true, detail: '模型对象均保留来源和互证记录' },
  ];
  state.catalog = { id: 'catalog-omnichannel-v1', modelVersion: 'V1', documentVersion: 'v1', publishedAt: timestamp, publishedBy: command.reviewer.trim(), reason: command.reason.trim(), immutable: true };
